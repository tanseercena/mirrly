...
class EnsureBilling
{
    ...
    public static function authorizeCharge(Session $session, array $config)
    {
        $confirmationUrl = self::requestPayment($session, $config);
    ...
    public static function getSubscriptions(Session $session)
    {
        return self::activeSubscriptions($session);
    }
    ...
    private static function activeSubscriptions(Session $session): array
    {
        $responseBody = self::queryOrException($session, self::RECURRING_PURCHASES_QUERY);
        $subscriptions = $responseBody["data"]["currentAppInstallation"]["activeSubscriptions"];
