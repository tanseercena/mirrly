...
export function MonthlyUsageCard() {
    ...
    const fileStorageUsed = calculateUsage(fileStorageLimit, file_storage_limit);
    const fileStorageUsedFormatted = formatStorageSize(fileStorageUsed);
    const fileStorageLimitFormatted = isUnlimited(fileStorageLimit) ? t("settings.usage_overview.unlimited") : formatStorageSize(fileStorageLimit);
    ...
    const fileSizeLimitFormatted = isUnlimited(mbPerFileLimit) ? t("settings.usage_overview.unlimited") : formatStorageSize(mbPerFileLimit);

    const renderProgressBar = (label, usage, limit, formattedUsage, formattedLimit) => {
        ...
        if (isLimitUnlimited && isUsageUnlimited) {
            return (
                <>
                    <Text variant="headingSm" as="h6">{label}</Text>
                    <div style={{marginTop: "5px"}}></div>

                    <InlineStack gap="300">
                        <div style={{width: "12%"}}>
                            <Text variant="bodySm" as="p">{t("settings.usage_overview.unlimited")}</Text>
                        </div>
                        <div style={{width: "86%"}}>
    ...
    const digitalProductsUsed = calculateUsage(digitalProductsLimit, digital_products_limit);
    const digitalLotteriesUsed = calculateUsage(digitalLotteriesLimit, digital_lotteries_limit);
        ...
        <Card>
            <InlineGrid columns="1fr auto">
                <div>
                    ...
                    {store?.subscription?.plan?.name && store?.subscription?.next_reset_date && (
                        <Text variant="bodySm" as="p" color="subdued">
                            {getTranslatedPlanName(store.subscription.plan.name)} {t("settings.usage_overview.plan_until")} {formatDate(store.subscription.next_reset_date)}
                        </Text>
                    )}
                </div>

                {store?.subscription?.plan?.name === 'unlimited' && store?.subscription?.status === 'active' ? (
                    <div onClick={handlePricing}>
                        <Link url="#">{t("settings.usage_overview.go_to_plans")}</Link>
                    </div>
                ) : (
                    <div onClick={handlePricing}>
                        <Link url="#">{t("settings.usage_overview.upgrade_plan")}</Link>
                    </div>
                )}
            </InlineGrid>
            ...
            <div style={{marginTop: "20px"}}></div>
            <BlockStack>
                {renderProgressBar(t("settings.usage_overview.orders_per_month"), ordersUsed, ordersLimit, ordersUsed, ordersLimit)}
                {renderProgressBar(t("settings.usage_overview.digital_products"), digitalProductsUsed, digitalProductsLimit, digitalProductsUsed, digitalProductsLimit)}
                {renderProgressBar(t("settings.usage_overview.digital_lotteries"), digitalLotteriesUsed, digitalLotteriesLimit, digitalLotteriesUsed, digitalLotteriesLimit)}
                {renderProgressBar(t("settings.usage_overview.storage"), fileStorageUsed, fileStorageLimit, fileStorageUsedFormatted, fileStorageLimitFormatted)}
