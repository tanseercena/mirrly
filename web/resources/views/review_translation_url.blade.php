...
<body>
    <div class="container">
        ...
        <!-- Header -->
        <div class="header">
            <h1>Translation Template Review</h1>
            <span class="status-badge">{{($reviewTemplate->status === 'approved' || $reviewTemplate->status === 'not_approved' || $translation->template_status === 'published' || $translation->template_status === 'not_approved') ? 'Review Completed' : 'Pending Review'}}</span>
        </div>
        ...
        <div class="review-info">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <span class="info-value">{{ $translation->title ?? 'Translation Template' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Created By</span>
                    <span class="info-value">{{ $parentTemplate->store->shopify_domain ?? 'John Doe' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Created Date</span>
                    <span class="info-value">{{ $translation->created_at->format('M d, Y H:i') ?? 'Dec 15, 2024 14:30' }}</span>
                </div>
                <div class="info-item">
                    <span class="info-label">Template ID</span>
                    <span class="info-value">#{{ $translation->id ?? '12345' }}</span>
                </div>
            </div>
        ...
        <!-- Comment Section -->
        @if(!in_array($translation->template_status, ['published', 'not_approved']) && !in_array($reviewTemplate->status, ['approved', 'not_approved']))
        ...
        <div class="template-preview">
            ...
            <div class="preview-content">
                <div class="template-html">
                    ...
                    {!! $translation->html ?? '<h2>Welcome to Our Service!</h2><p>Thank you for joining us. Click <a href="https://example.com/welcome">here</a> to get started.</p><p>Best regards,<br>The Team</p>' !!}
                </div>
            </div>
        </div>
        ...
        @if(!in_array($translation->template_status, ['published', 'not_approved']) && !in_array($reviewTemplate->status, ['approved', 'not_approved']))
            ...
            <div class="action-buttons">
                ...
                <form method="POST" action="{{ route('translation.approve', $reviewTemplate->id) }}" style="display: inline;">
                    @csrf
                ...
                <form method="POST" action="{{ route('translation.reject', $reviewTemplate->id) }}" style="display: inline;">
                    @csrf
